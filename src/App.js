...
const video = props => {
  return (
    <div key={props.id} className="item-container">
      <video autoPlay muted loop src={props.media_url} className="item"></video>
    </div>
  )
}
...
class App extends Component {
  ...
  render() {
    if (this.state.error) return <p>Oops! Unable to load the images. Please refresh!</p>
    return this.state.loading ? <p><img src="/loading.svg" alt="loading..." /></p> : (
      <div>
          {this.state.items.map(item => {
            switch (item.media_type) {
              case 'VIDEO': return video(item)
              default: return image(item)
            }
          })}
      </div>
    );
